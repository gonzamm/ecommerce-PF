  ...
  describe("PRODUCTOS", () => {
    ...
    it("GET producto existente deberia retornar un status 200", async () => {
      let response = await request.get(
        "/api/productos/622bb1e842c74eee8c2db801"
      );
      expect(response.status).to.eql(200);
    });
    it("GET producto inexistente deberia retornar un status 400", async () => {
      let response = await request.get(
        "/api/productos/622bb1e842c74eee8c2db802"
      );
    ...
    it("POST con producto nuevo deberia retornar un status 200", async () => {
      const newPto = {
        ...
        descripcion: "Mochila Adidas, color azul",
        categoria: "Mochilas",
        thumbail:
          "https://static.dafiti.cl/p/adidas-originals-3318-381538-1-product.jpg",
        precio: "2500.30",
        stock: 15,
      };
    ...
    it("DELETE a un producto inexistente deberia retornar un status 400", async () => {
      let response = await request.delete(
        "/api/productos/61e0931bbcf2d104d8538206"
      );
      expect(response.status).to.eql(400);
    });
  });
